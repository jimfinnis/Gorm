// Noise class core Copyright 2001 Ken Perlin
...
public final class Noise {
	...
	/**
	 * The standard noise scale
	 */
	private static final double DEFAULT_SCALE = 0.01;

	/**
	 * 
	 * 2D fractal scaled noise
	 * 
	 * @param x
	 * @param y
	 * @param map
	 *            which noise set we're looking at
	 * @param scale
	 *            a scale factor; 1 is good for Minecraft maps, higher for
	 *            smaller scale
	 * @param octaves
	 *            number of iterations
	 * @param persistence
	 *            see Mandelbrot's work - 0-1, high means rough
	 * @return value in the range 0 to 1
	 */
	public static double noise2Dfractal(int x, int y, int map, double scale,
		...
		for (int i = 0; i < octaves; i++) {
			t += noise(x * DEFAULT_SCALE * f, y * DEFAULT_SCALE * f);
			f *= 2;
			p *= p;
		}
		return (clip(t)+1.0)*0.5;
	}

	/**
	 * multi-octave noise has a gaussian distribution, so may go out of range.
	 * @param t
	 * @return
	 */
	...
	/**
	 * produce a boolean out of 2D noise, which is fuzzy (in that a random fuzz
	 * is added to the generated noise before it is tested against the
	 * threshold)
	 * 
	 * @param x
	 * @param y
	 ...
	 *            smaller scale
	 * @param thresh
	 *            the threshold 0 means nothing will be in the map, 1 means
	 *            everything.
	 * @param fuzz
	 *            the amount of fuzz to be added, 0-1
	 * @return
	 */
	public static boolean fuzzyBooleanNoise(double x, double y, int map, double scale,
			double thresh, double fuzz) {
		x += (double)map*10000;
		y += (double)map*10000;
		double d = noise(x * DEFAULT_SCALE * scale, y * DEFAULT_SCALE
				* scale);
		d += r.nextDouble() * fuzz - fuzz * 0.5;
		return (d + 1) * 0.5 < thresh;
	}
	
	
	/**
	 * helper for scaling ints by noise values
	 * 
	 */
	public static int scale(int val,double sc){
		return (int)(((double)val)*sc);
	}
